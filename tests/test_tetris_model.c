...
static void test_seven_bag_contains_every_piece(void) {
    tetris_model_t model;
    bool seen[8] = { false };
    tetris_model_init(&model, 2026);
    for (int i = 0; i < 7; i++) {
        assert(model.bag[i] >= TETRIS_PIECE_I && model.bag[i] <= TETRIS_PIECE_L);
        assert(!seen[model.bag[i]]);
        seen[model.bag[i]] = true;
    }
}

static void test_ghost_piece_reaches_floor(void) {
    tetris_model_t model;
    tetris_model_init(&model, 31415);
    memset(model.board, 0, sizeof(model.board));
    model.piece = TETRIS_PIECE_O;
    model.rotation = 0;
    model.x = 3;
    model.y = 0;
    assert(tetris_model_ghost_y(&model) == 18);
}
...
int main(void) {
    test_initial_state();
    test_seven_bag_contains_every_piece();
    test_ghost_piece_reaches_floor();
