...
3. **合并完整固件**：在 `build/` 下用 `esptool merge_bin -o FoloToy-AI-Passport-full.bin @flash_args --flash_size 8MB` 合并 bootloader + partition-table + app。`@flash_args` 提供分区地址；后面的 `--flash_size 8MB` 覆盖 `HEADER_FLASHSIZE_UPDATE` 生成的 `detect`，不改 `sdkconfig.defaults`，也不填充整片 8MB。
4. **发布**：
