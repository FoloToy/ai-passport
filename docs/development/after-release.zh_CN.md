...
把它合并进总结；若没有，直接总结。归档完成后，**同时更新承载该应用的分支的根 README**，让归档的应用
登记到承载它的分支上。对每个分支**各自处理**：没有根 README 的分支创建它（登记新归档的应用），
已有根 README 的分支**主动更新它**（加进新应用；若 README 是多项目目录，把它作为介绍的项目之一）。
