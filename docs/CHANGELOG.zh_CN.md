...
- 规定发布 Release 后，fork owner 在修改 Release 标题与说明之外，还要同步更新 fork 根目录的 `README.md`（fork 自己在 `main` 上的项目目录，一个 fork 可能承载多个开发项目），让新发布的应用能从仓库落地页被检索到。扩展了 `docs/development/CI-build-and-release.md` 的发布流程与 `docs/fork-guide.md` 的 fork README 约定。
