...
Release tag/commit context: this experience was captured after publishing the
"Voice Keychain" firmware to the AI Passport community market.

## What was learned

Publishing firmware to the AI Passport community does not end at upload. A
fork's release is only complete when the publishing flow is closed with three
follow-up tracks, and each of those has reusable, general lessons that apply to
any AI Passport release.

### 1. Confirm the publish destination before routing
...
### 2. The community upload needs three inputs, and the split matters
...
### 3. Authorize through the official site; never handle credentials
...
### 4. Post-release follow-up has its own consent gates
