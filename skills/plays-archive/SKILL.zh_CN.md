...
3. **无论是否归档，README 整理都必须进行。** 归档到 `plays/` 是**可选的**（开发者可以拒绝）；但仍要更新
   承载分支上的 README 与 fork `main` 的 README，让应用登记到它开发的地方。**各自处理**承载分支与
   `main`（不是一个合并判断）：
   - **承载分支**（承载该应用的 `feature/*` 分支）：若没有根 README，在该分支创建双语
     `README.md` / `README.zh_CN.md` 配对；若有，**主动更新它**（不要停在提示层面），新增或刷新该应用
     自身的说明。
   - **fork `main`**：更新根目录 `README.md` / `README.zh_CN.md`，让发布的应用能从仓库落地页被检索到。
     `main` 根 README 是 **fork 各项目的目录**：它**完整囊括**每个项目自己 README 的内容——完整说明该应用
     做什么、怎么用（交互、模式、按键、持久化与说明），而不是一行简介 + 分支链接。内容取自承载分支的
     README，让 `main` 呈现每个项目的完整、可读的说明。

4. **建议：这些 README 更新直接合并，不开 PR。** fork 根 README 与承载分支的根 README 是 fork 自有内容
   （不属于上游提案），直接 commit 到分支 / fork `main` 即可，不必等 review PR。只有当改动是要回到上游时
   才开 PR。
